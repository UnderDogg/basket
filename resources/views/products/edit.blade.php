        ...
        @forelse($products as $group)
            ...
            @forelse($group->getProducts() as $productData)
                <tr class="{{$productData->getId()}}">
                    ...
                    <td><input type="number" class="form-control" data-fv-between-inclusive="true" data-fv-between-min="{{number_format($productData->getDeposit()->getMinimumPercentage(), 0)}}" data-fv-between-max="{{$productData->getDeposit()->getMaximumPercentage()}}" name="min-{{$productData->getId()}}" data-fv-lessthan="true" data-fv-lessthan-value="max-{{$productData->getId()}}" value="@if(array_has($limits, $productData->getId())){{number_format($limits[$productData->getId()]['min_deposit_percentage'], 2)}}@else{{number_format($productData->getDeposit()->getMinimumPercentage(), 2)}}@endif"></td>
                    <td><input type="number" class="form-control" data-fv-between-inclusive="true" data-fv-between-min="{{number_format($productData->getDeposit()->getMinimumPercentage(), 0)}}" data-fv-between-max="{{$productData->getDeposit()->getMaximumPercentage()}}" name="max-{{$productData->getId()}}" data-fv-greaterthan="true" data-fv-greaterthan-value="min-{{$productData->getId()}}" value="@if(array_has($limits, $productData->getId())){{number_format($limits[$productData->getId()]['max_deposit_percentage'], 2)}}@else{{number_format($productData->getDeposit()->getMaximumPercentage(), 2)}}@endif"></td>
                <tr>
            @empty
                <tr>
